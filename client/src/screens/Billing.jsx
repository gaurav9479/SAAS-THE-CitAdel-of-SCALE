...
export default function Billing() {
  ...
  const mailto =
    "mailto:sales@example.com?subject=Plan%20change&body=Please%20change%20our%20plan.";
  ...
  const applyPlan = async (planId) => {
    ...
    setMessage("");
    try {
      const { data } = await api.patch(`/api/orgs/${orgId}/plan`, {
        plan: planId,
      });
      if (data?.organization) {
        setUser({ ...user, organization: data.organization });
        setMessage(
          `Plan updated to ${data.organization.plan.toUpperCase()} (prototype).`
        );
      }
    } catch (e) {
      setMessage(
        e.response?.data?.message ||
          "Failed to change plan, please email sales."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="p-6 space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-sm text-gray-600">
          Select a plan to simulate upgrading/downgrading. Payments are not
          wired; admins can change plan directly or contact sales.
        </p>
      </div>
      {message && (
        <div className="rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-2 text-emerald-800 text-sm">
          {message}
        </div>
      )}
        ...
        {PLANS.map((plan) => (
          <div
            ...
            className={`rounded-xl border ${
              plan.id === selected ? "border-emerald-600" : "border-gray-200"
            } bg-white shadow-sm p-4 flex flex-col gap-3`}
          >
            <div className="space-y-1">
              <div className="text-xs uppercase tracking-wide text-gray-500">
                {plan.id === user?.organization?.plan
                  ? "Current plan"
                  : "Available"}
              </div>
      ...
      <div className="text-sm text-gray-600">
        Payments are not integrated. Use the buttons above to prototype plan
        changes or email sales.
      </div>
    </div>
  );
}
